...
var db = {
	...
	init:function(){
		...
		this.database.execute('CREATE TABLE IF NOT EXISTS dbVersion( '+
	...
	,checkTable:function(){
		...
		this.database.execute('CREATE TABLE IF NOT EXISTS propgroup( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'number integer, '+
			'filename text,  '+
			'recordtype integer,  '+
		...
		this.database.execute('CREATE TABLE IF NOT EXISTS property( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'number integer, '+
			'intno integer,  '+
			'propertyno integer,  '+
			...
			'RENTFREE integer, '+
			'RENTFREEDAYS real, '+
			'RENTFREEMTHS integer, '+
			'CARPARKS integer, '+
			'LEGALCOCKLOFT integer, '+
			'DECORATION text, '+
			'PROPVIEW integer, '+
			'RAISEDFLOOR integer, '+
			'FACINGLIFT integer, '+
		...
		this.database.execute('CREATE TABLE IF NOT EXISTS propertyFile( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'propgroupno integer, '+
			'propertyno integer,  '+
			'filename text,  '+
		...
		this.database.execute('create table if not exists estate( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'NUMBER integer, '+
			'ESTATENO integer, '+
			'ENGLISHNAME text, '+
			...
			'ATTACHMENT blob, '+
			'createby integer);');
		this.database.execute('create table if not exists estatefile( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'ESTATENO integer, '+
			'ATTACHMENT blob, '+
			'filename text, '+ 
			...
			'createby integer);');
		this.database.execute('CREATE TABLE IF NOT EXISTS newdev( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'number integer, '+
			'englishname text,  '+
			'chinesename text,  '+
		...
		this.database.execute('CREATE TABLE IF NOT EXISTS trans( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			...
			'transacdate integer,  '+
			'createby integer);');
		this.database.execute('CREATE TABLE IF NOT EXISTS doctype( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'number integer, '+
			'chinesename text,  '+
			'englishname text,  '+ 
			'deletedate integer DEFAULT 0, '+
			'updateinfo text DEFAULT \'0\');');
		this.database.execute('CREATE TABLE IF NOT EXISTS document( '+
			'id INTEGER PRIMARY KEY AUTOINCREMENT, '+
			'attachment blob, '+
			'c_subject text,  '+
			'e_subject text,  '+
		...
		while (rows.isValidRow())
			{
				...
				for (var i=0;i<fieldCount;i++){
					rowObj[rows.fieldName(i).toLowerCase()]=rows.field(i);
				}
			...
			if (fieldInfo[k.toLowerCase()]){
				...
				value_end+=",obj['"+k+"']";
					if (typeof obj[k] == 'object')
						...
						if (fieldInfo[k.toLowerCase()]=='text'){
