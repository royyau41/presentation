...
//Rare exceptions to indicate a bug in the _presentation code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_presentation = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_presentation]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.presentation.module"
